...
*		      unitario   :H='P. Unit.'  :W=fen() :V=fresul() :F,;
*		      por_desc   :H='%Desc.'    :W=fen() :V=fdesc() 	AND fresul(2) :P="@ 99.99":F,;
*	    	  Total      :H='Total' 				:P="@ 999,999.99" ;
*	    	  		:W=.F.;
*		NODELETE NOMENU
...
REPLACE total 	WITH ROUND(cantidad * unitario,2) - descuento
...
mtotal = ROUND(cantidad * unitario,2)

IF t_impuesto = "%"
	mneto = ROUND(mtotal / (1 + (m.porivari + p_impuesto ) / 100),2)
	REPLACE impuesto	WITH ROUND(mneto * p_impuesto / 100,2)
ELSE
...
FUNCTION agregar
PRIVATE anreg, renglon

SELECT (tdetalle)
anreg = RECNO()
COUNT TO renglon	FOR !DELETED()
GO BOTTOM
