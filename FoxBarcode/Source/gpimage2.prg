...
DEFINE CLASS gpImage2 AS CUSTOM
  ...
  PROCEDURE CreateFromGraphics(tnGraphics)
    ...
    lnWidth = float2int(buf2dword(SUBSTR(m.lcRectF,9,4)))
    lnHeight = float2int(buf2dword(SUBSTR(m.lcRectF,13,4)))
    ...
    *!*	this.Destroy()
    lnImage = 0
    THIS.STAT = GdipCreateBitmapFromGraphics(lnWidth, lnHeight, tnGraphics, @lnImage)
    THIS.NativeImage = lnImage
  ENDPROC
  ...
  PROCEDURE CloneArea(tnX, tnY, tnWidth, tnHeight, tcPixelFormat)
    IF (VARTYPE(tnX) != 'N') OR (VARTYPE(tnY) != 'N')
      ERROR 11
    ENDIF
    IF (VARTYPE(tnWidth) != 'N') OR (VARTYPE(tnHeight) != 'N')
      ERROR 11
    ...
    gpdstBitmap = 0
    THIS.STAT = GdipCloneBitmapAreaI(tnX, tnY, tnWidth, tnHeight, ;
  ...
  * tnColorDepth - number of colors in bits per pixel (1,4,8,16,24,32,48,64), optional.
  ...
  PROCEDURE GetPixelColor(tnX, tnY)
    ...
    argb = 0
    THIS.STAT = GdipBitmapGetPixel(THIS.nativeImage, tnX, tnY, @argb)
  ...
  PROCEDURE SetPixel(tnX, tnY, tnColor, tnAlpha)
    IF (VARTYPE(tnX) != 'N') OR (VARTYPE(tnY) != 'N') OR (VARTYPE(tnColor) != 'N')
      ERROR 11
    ENDIF
    ...
    THIS.STAT = GdipBitmapSetPixel(THIS.nativeImage, tnX, tnY, argb)
  ENDPROC
...
DEFINE CLASS Graphics AS CUSTOM
  ...
  *!*	STRING FUNCTIONS
  ...
  PROCEDURE SetRect(tnX, tnY, tnWidth, tnHeight)
    ...
    IF !(VARTYPE(m.tnX) + VARTYPE(m.tnY) + VARTYPE(tnWidth) + VARTYPE(tnHeight) == 'NNNN')
      ERROR 11	&& function argument
    ENDIF
    THIS.Rectf = rectf(m.tnX, m.tnY, tnWidth, tnHeight)
  ENDPROC
  ...
  *!*	IMAGE ATTRIBUTES
  ...
  PROCEDURE CreateFromGraphics(tnGraphics)
    ...
    *!*	this.Destroy()
    nImage = 0
    THIS.STAT = GdipCreateBitmapFromGraphics(lnWidth, lnHeight, tnGraphics, @nImage)
  ENDPROC
...
FUNCTION POINTF(tnX,tnY) && adapted from RECTF found in www.news2news.com
  RETURN dword(Int2Float(tnX)) + dword(Int2Float(tnY))
ENDFUNC

FUNCTION RECTF(tnX,tnY,tnWIDTH,tnHEIGHT) && found in www.news2news.com
  RETURN dword(Int2Float(tnX)) + dword(Int2Float(tnY)) +;
    dword(Int2Float(tnWIDTH)) + dword(Int2Float(tnHEIGHT))
ENDFUNC
  ...
  * converts FoxPro numeric to 32-bit float form
  ...
  DO CASE
    ...
    OTHERWISE
      RETURN 0
  ENDCASE
  exponent = FLOOR(LOG(tnNum)/LOG(2))
  mantissa = (tnNum - 2^exponent)* 2^(REAL_MANTISSA_SIZE - exponent)
...
FUNCTION Float2Int(tnNum) && By Anatoly Mogylevets - found in www.fox.wikis.com
  * converts 32-bit float form to FoxPro numeric
  LOCAL sgn, exponent, mantissa
  sgn = IIF(BITTEST(tnNum,31), -1,1)
  exponent = BITRSHIFT(BITAND(tnNum, EXPONENT_MASK),;
    REAL_MANTISSA_SIZE) - REAL_BIAS
  mantissa = BITAND(tnNum,;
...
FUNCTION MakePointsFSequence(taIntPoints)
  *** Creates a String containing all PointsF of the received array
  ...
  lcPointsFSequence = ""
  FOR ln = 1 TO ALEN(taIntPoints,1)
    lcPointF = PointF(taIntPoints(ln,1), taIntPoints(ln,2))
...
FUNCTION MakeColorMatrix(taColMatrix)
  *** Creates a String containing ColorMatrix from the 5x5 received array
  ...
  FOR lnRow = 1 TO 5
    FOR lnCol = 1 TO 5
      lcMatrix = lcMatrix + dword(Int2Float(taColMatrix(lnRow, lnCol)))
    ENDFOR
  ENDFOR
  RETURN lcMatrix
ENDFUNC
...
FUNCTION MakeColorMap(taColorMap)
  *** Creates a String containing ColorMapt from the 4 column Array
  ...
  lcColorMap = ""
  *!*		FOR ln = 1 TO ALEN(taColorMap,1)
  *!*			lnargbOld = MakeARGB(taColorMap(ln,1), taColorMap(ln,3))
  *!*			lnargbNew = MakeARGB(taColorMap(ln,2), taColorMap(ln,4))
